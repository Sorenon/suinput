...
use windows_sys::{
    core::GUID,
    Win32::Devices::HumanInterfaceDevice::{GUID_DEVINTERFACE_KEYBOARD, GUID_DEVINTERFACE_MOUSE},
};
...
fn main() {
    ...
    for (device_interface_name, _) in
        mice_device_info_set.iter_device_interfaces(GUID_DEVINTERFACE_MOUSE)
    {
    ...
    for (device_interface_name, device) in
        kbd_device_info_set.iter_device_interfaces(GUID_DEVINTERFACE_KEYBOARD)
    {
